"""
Module contains utility functions for the project. One of these is to generate a random name for the run, which we base
on Docker's approach when naming containers. The code is taken from:
https://github.com/moby/moby/blob/master/pkg/namesgenerator/names-generator.go

This has the following licence:
...
def mem_get_info(device):
    """
    GPU memory for a particular device.
    Have to use context as bug in older versions of pytorch: https://github.com/pytorch/pytorch/issues/76224
    """
...
def get_name():
    """
    Gets random name based on adjectives and people lists (based on how Docker does it).
    See module docs for more information.
    """
    rand = os.urandom(4)
    rand_seed = int(binascii.hexlify(rand), 16)
    rng = np.random.RandomState(rand_seed)
    return rng.choice(adjectives), rng.choice(people)


def get_logger(*, internal_log_name=None, log_file_name="run_log.log", first_log_handler=None):
    """
    Get a logger with both a stream and file handler and sets sensible defaults.
    """
    internal_log_name = internal_log_name or __name__
    logger = logging.getLogger(internal_log_name)

    # first handler (stream handler by default)
    if first_log_handler is None:
        sh = logging.StreamHandler(sys.stdout)
    else:
        sh = first_log_handler
    fmt = logging.Formatter(f"{internal_log_name} - %(asctime)s - %(levelname)s - %(message)s",
                            datefmt="%Y-%m-%d %H:%M:%S")
    sh.setFormatter(fmt)
...
def noop(*args, **kwargs): pass  # a function that does nothing! (useful where a function is expected).


def set_paths_relative_to_given_pth(dict_in, pth, keys_to_change: Iterable):
    """
    Updates the path variables of dict_in so that they are stored relative to the given path. This includes values
    that actually are Python lists of paths as well as direct strings.
    This skips over paths that are already defined as absolute or are not defined (i.e., stored as None).

    Note modifies the dictionary in place! Probably does not work on Windows.

    :param dict_in: the dictionary holding path variables.
    :param pth: the path to add to the beginning of current paths if not absolute already.
    :param keys_to_change: the keys corresponding to the path variables (either str or lists of str)
        that should be changed.
    """
    changes_all = {}
    for file_attr_name in keys_to_change:

        try:
            cur_file_path_or_list_of_fpaths = dict_in[file_attr_name]
        except KeyError:
            continue

        if isinstance(cur_file_path_or_list_of_fpaths, str):
            cur_file_path_or_list_of_fpaths = [cur_file_path_or_list_of_fpaths]
            unpack_flag = True
        elif isinstance(cur_file_path_or_list_of_fpaths, list):
            unpack_flag = False
        else:
        ...
        out = []
        changes = []
        for fpath in cur_file_path_or_list_of_fpaths:
            if osp.isabs(fpath) or (fpath is None):
                out.append(fpath)

            else:
                new_file_path = osp.join(pth, fpath)

                out.append(new_file_path)
                changes.append((fpath, new_file_path))

        if unpack_flag:
            out = out[0]

        dict_in[file_attr_name] = out
        changes_all[file_attr_name] = changes

    return changes_all


def check_paths_at_keys_absolute(dict_in, keys_to_check):
    """
    Checks that all the paths at the given keys are absolute paths. If not, returns False.
    """
...
def is_env_variable_true(name: str, default_value: Optional[bool] = None) -> bool:
    """
    Checks if an environment variable is set to a truthy value.
    Adapted from https://stackoverflow.com/questions/63116419/evaluate-boolean-environment-variable-in-python
    """
    true_ = {'true', '1', 't', 'y', 'yes'}
    false_ = {'false', '0', 'f', 'no', 'n'}
...
class NumpyFloatValuesEncoder(json.JSONEncoder):
    """
    Allows numpy float32 values to be serialized in json.
    Adopted from: https://stackoverflow.com/questions/64154850/convert-dictionary-to-a-json-in-python
    """
